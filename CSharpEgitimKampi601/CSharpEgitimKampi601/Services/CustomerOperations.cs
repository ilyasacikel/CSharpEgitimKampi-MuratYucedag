...
namespace CSharpEgitimKampi601.Services
{
    public class CustomerOperations
    {
        ...
        {
            var connections = new MongoDbConnections();
            var customerCollections = connections.GetCustomersCollection();
            var deletedCustomer = Builders<BsonDocument>.Filter.Eq("_id", ObjectId.Parse(id));
            customerCollections.DeleteOne(deletedCustomer);
        }

        public void UpdateCustomer(Customer customer)
        {
            var connection = new MongoDbConnections();
            var customerCollections = connection.GetCustomersCollection();
            ...
            customerCollections.UpdateOne(filter, updatedValue);
        }
